...
#
# Modifs : plus d'apply() de groupby → on itère proprement sur les groupes,
# et on passe observed=False explicitement pour éviter les FutureWarning.
...
def _build_preprocessor(cat_cols: List[str], num_cols: List[str]) -> ColumnTransformer:
    # Encodage catégoriel + normalisation numérique
    try:
...
def train_and_compare(
    ...
    Retourne :
      ...
      - prédictions du meilleur modèle (pour fairness)
    """
...
def fairness_tables(
    ...
    On évite groupby.apply (qui change) et on itère sur les groupes → plus clair et sans warning.
    """
    if groups is None:
    ...
    for col in groups:
        ...
        rows = []
        # observed=False = comportement actuel de pandas; on l'explicite pour éviter le FutureWarning
