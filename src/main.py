...
if "MonthlyIncome" in df.columns:
    ...
    if "Gender" in fair_tabs:
        ratios["MAPE_Female_over_Male"] = float(parity_ratio(fair_tabs["Gender"], "Gender", "Female", "Male"))
    if "MaritalStatus" in fair_tabs:
        ratios["MAPE_Married_over_Single"] = float(parity_ratio(fair_tabs["MaritalStatus"], "MaritalStatus", "Married", "Single"))
    if "age_group" in fair_tabs:
        ag = fair_tabs["age_group"]
        try:
            a = float(ag.loc[ag["age_group"] == "<30", "MAPE (%)"].iloc[0])
            b = float(ag.loc[ag["age_group"] == "40-49", "MAPE (%)"].iloc[0])
